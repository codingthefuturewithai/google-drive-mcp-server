...
async def tool_with_defaults_pattern(
    ...
    - String return type (project convention: tools return str, not Dict)
    ...
    """
    return f"Required: {required_param}, Int: {optional_int}, Flag: {optional_bool}"
...
async def tool_with_validation_pattern(
    ...
    return f"Valid range: {min_value}-{max_value}"
...
async def computation_tool_pattern(
    ...
    - Formatted text response (project convention: return str)
    ...
    Returns:
        Formatted text with computation results
    """
    import time
    import asyncio
    ...
    return f"Computed sum(0..{n}) = {result} in {computation_time*1000:.2f}ms ({n} operations)"
