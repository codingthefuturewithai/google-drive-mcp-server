...
The Google Drive implements a unified logging system that captures both tool executions and internal application logs in a single, queryable database. The system features:
...
```python
from google_drive.log_system import set_correlation_id, get_correlation_id
...
```python
from google_drive.log_system import get_tool_logger
from mcp.server.fastmcp import Context
...
```python
from google_drive.decorators import tool_logger
from mcp.server.fastmcp import Context

@tool_logger
async def my_tool(data: str, ctx: Context = None) -> str:
...
```python
from google_drive.log_system import CorrelationContext, get_tool_logger
...
```python
from google_drive.log_system.destinations import SQLiteDestination
