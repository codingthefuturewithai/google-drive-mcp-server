"""Configuration management page for Google Drive Admin UI
...
try:
    from google_drive.ui.lib.components import (
        render_info_card,
        render_warning_banner,
        render_config_section
    )
    from google_drive.ui.lib.utils import (
        load_configuration,
...
def render_configuration_form(config):
    ...
    with st.form("config_form"):
        ...
        with col1:
            ...
            server_name = st.text_input("Server Name", value=server_config.get("name", "Google Drive"))
...
def handle_save_config(server_name, server_port, log_level, log_retention, current_config):
    ...
    new_config = {
        "server": {
            "name": server_name,
            "description": current_config.get("server", {}).get("description", "Google Drive - MCP server with tools and integrations"),
            "port": server_port,
            "log_level": log_level,
...
def handle_export_config(config):
    ...
    st.download_button(
        ...
        data=config_json,
        file_name="google_drive_config.json",
...
def render_environment_variables():
    ...
    env_vars = {
        "GOOGLE_DRIVE_LOG_LEVEL": "INFO",
        "GOOGLE_DRIVE_PORT": "3001",
        "GOOGLE_DRIVE_CONFIG_PATH": system_info.get("config_path", "~/.config/google_drive"),
        "GOOGLE_DRIVE_LOG_PATH": system_info.get("log_path", "~/.local/share/google_drive"),
...
def main():
    ...
    # Page header
    st.title("⚙️ Google Drive Configuration")
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=config_yaml,
            file_name="google_drive_config.yaml",
