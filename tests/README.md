# Testing Guide for Google Drive
...
uv run pytest tests/ --cov=google_drive --cov-report=term --cov-report=html
...
uv run pytest tests/ --cov=google_drive --cov-report=html
...
import pytest
from google_drive.tools.my_tools import my_new_tool
...
uv run pytest tests/ --cov=google_drive --cov-fail-under=40

# Generate coverage report for CI artifacts
uv run pytest tests/ --cov=google_drive --cov-report=xml
```
...
4. Check coverage hasn't decreased: `uv run pytest tests/ --cov=google_drive`
